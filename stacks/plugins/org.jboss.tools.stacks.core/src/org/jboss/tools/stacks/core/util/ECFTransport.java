package org.jboss.tools.stacks.core.util;
...
import org.jboss.tools.stacks.core.Messages;
...
public class ECFTransport {
	...
	public long getLastModified(URL location) throws CoreException {
		...
		throw new CoreException(new Status(IStatus.ERROR, StacksCoreActivator.PLUGIN_ID, Messages.ECFExamplesTransport_IO_error, null));
	}
	...
	public IStatus download(String name, String url, OutputStream destination, IProgressMonitor monitor) {
		...
		return new Status(IStatus.ERROR, StacksCoreActivator.PLUGIN_ID,  Messages.ECFExamplesTransport_IO_error, null);
	}
	...
	public IStatus performDownload(String name,String toDownload, OutputStream target, IConnectContext context, IProgressMonitor monitor) throws ProtocolException {
		...
		if (factory == null)
			return new Status(IStatus.ERROR, StacksCoreActivator.PLUGIN_ID, Messages.ECFExamplesTransport_IO_error);
					...
					if (monitor != null) {
						if (monitor.isCanceled()) {
							try {
								...
								IStatus status = new Status(IStatus.WARNING, StacksCoreActivator.PLUGIN_ID,Messages.ECFTransport_Operation_canceled);
								StacksCoreActivator.getDefault().getLog().log(status);
							}
							return;
						}
	...
	public IConnectContext getConnectionContext(String xmlLocation, boolean prompt) throws UserCancelledException, CoreException {
		...
		if (!prompt) {
			...
			try {
				...
				throw new CoreException(new Status(IStatus.ERROR, StacksCoreActivator.PLUGIN_ID, msg, e));
			}
		}
		...
		IProvisioningAgent agent = (IProvisioningAgent) ServiceHelper.getService(StacksCoreActivator.getDefault().getBundleContext(), IProvisioningAgent.SERVICE_NAME);
		...
		if (loginDetails.saveResult()) {
			...
			try {
				...
				String msg = Messages.ECFExamplesTransport_Internal_Error;
				throw new CoreException(new Status(IStatus.ERROR, StacksCoreActivator.PLUGIN_ID,  msg, e1));
			} catch (IOException e) {
				String msg = Messages.ECFExamplesTransport_Internal_Error;
				throw new CoreException(new Status(IStatus.ERROR, StacksCoreActivator.PLUGIN_ID, msg, e));
			}
	...
	private IStatus convertToStatus(Exception e) {
		...
		if (e instanceof UserCancelledException)
			return new Status(IStatus.CANCEL, StacksCoreActivator.PLUGIN_ID, e.getMessage(), e);
		return new Status(IStatus.ERROR, StacksCoreActivator.PLUGIN_ID, e.getMessage(), e);
	}
	...
	private synchronized ServiceTracker getFileTransferServiceTracker() {
		if (retrievalFactoryTracker == null) {
			retrievalFactoryTracker = new ServiceTracker(StacksCoreActivator.getDefault().getBundleContext(), IRetrieveFileTransferFactory.class.getName(), null);
	...
	private boolean startBundle(String bundleId) {
		PackageAdmin packageAdmin = (PackageAdmin) ServiceHelper.getService(StacksCoreActivator.getDefault().getBundleContext(), PackageAdmin.class.getName());
